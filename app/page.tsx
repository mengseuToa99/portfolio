...
import { Github, Linkedin, Package, Instagram, BookOpen, Menu, X } from 'lucide-react';
...
export default function Portfolio() {
  ...
  const scrollToSection = (sectionIndex: number) => {
    if (isMobile) {
      // For mobile, scroll to the section vertically
      const sections = ['about', 'experience', 'projects'];
      ...
      // For desktop/tablet, use horizontal scrolling
      if (contentRef.current) {
        const { scrollWidth, clientWidth } = contentRef.current;
        const sectionWidth = (scrollWidth - clientWidth) / 2;
        const targetScrollLeft = sectionWidth * sectionIndex;
  ...
  // Handle mobile scroll for active section tracking
  useEffect(() => {
    if (!isMobile) return;
        ...
        <div className="relative min-h-screen">
          ...
          <div className="flex flex-col items-center justify-center min-h-screen p-6">
            ...
            <div
              ...
              className={`relative mb-6 ${isSmallMobile ? 'w-24 h-24' : 'w-32 h-32'}`}
            ...
            <h1 className={`${isSmallMobile ? 'text-2xl' : 'text-3xl'} font-bold text-slate-100 mb-4 text-center`}>
              Tai Mengseu
            </h1>
            <h2 className={`${isSmallMobile ? 'text-base' : 'text-lg'} text-slate-200 mb-6 text-center`}>Web Developer</h2>
            <p className={`text-slate-400 mb-12 ${isSmallMobile ? 'text-xs' : 'text-sm'} text-center px-4`}>
          ...
          <section id="experience" className="min-h-screen p-6">
            <div className="max-w-3xl mx-auto">
              ...
              <div className="space-y-6">
                <div className="group relative transition-all duration-300 hover:bg-slate-800/50 p-4 md:p-6 rounded-lg">
        ...
        <div className="relative flex min-h-screen">
          ...
          <div className={`${isTablet ? 'w-2/5' : 'w-1/2'} fixed left-0 top-0 h-screen flex flex-col justify-between p-3 sm:p-4 md:p-8 lg:p-12 xl:p-20`}>
            <div>
              ...
              <div
                ref={imageContainerRef}
                className={`relative mb-6 ${isTablet ? 'w-40 h-40' : 'w-48 h-48'} mx-auto lg:mx-0`}
                onMouseEnter={() => setIsHovering(true)}
                onMouseLeave={() => setIsHovering(false)}
              >
                {/* Energy field background */}
              ...
              <h1 className={`${isTablet ? 'text-4xl' : 'text-5xl'} font-bold text-slate-100 mb-4 lg:text-left`}>
                Tai Mengseu
              </h1>
                ...
                <section className="w-1/3 flex-shrink-0 p-3 sm:p-4 md:p-8 lg:p-12 xl:p-20 h-screen overflow-hidden">
                  <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto pr-4 space-y-6">
                      <div className="group relative transition-all duration-300 hover:bg-slate-800/50 p-4 md:p-6 -mx-4 md:-mx-6 rounded-lg">
